...
async function sendSlice(max) {
  ...
  for (const inbox of inboxes) {
    ...
    // business-hours gate — cold sends only inside the window; warmup already ran above
    const win = settings.sendWindow || { start: 9, end: 17 };
    const hr = new Date().getHours();
    if (hr < win.start || hr >= win.end) continue;
